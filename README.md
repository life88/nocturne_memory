...
2. 进入目录，优先运行 `python setup.py`（或 `python nocturne.py setup`）启动交互式安装向导。
3. 如果我不想走向导，再手动运行 `pip install -r backend/requirements.txt`，并复制 `.env.example` 为 `.env`。
4. 告诉我默认 SQLite 会存到 `~/.nocturne-memory/memory.db`，`~` 会自动展开，不需要我手动填写绝对路径。
5. 询问我使用的是哪个客户端（Claude/Cursor/Antigravity etc）。
   - 默认客户端统一指向 `nocturne.py`，args 使用 `["/absolute/path/to/nocturne.py", "stdio"]`
   - 如果是 **Antigravity (Windows)**：args 改为 `backend/mcp_wrapper.py`（解决 CRLF 问题）。
6. 可选：运行 `python nocturne.py config`，把生成的 MCP 配置模板给我复制。
```
...
- 安装 Python 依赖
- 创建 `.env`
- 初始化默认数据目录 `~/.nocturne-memory/`
- 输出可直接复制的 MCP 配置
- 可选安装 Claude Code skill（复制到 `~/.agents/skills`，并链接到 `~/.claude/skills`）

<details>
<summary><strong>🔧 手动安装（不走向导）</strong></summary>
...
`.env.example` 默认已经可直接使用：

```ini
# SQLite — 本地单机（默认）
DATABASE_URL=sqlite+aiosqlite:///~/.nocturne-memory/memory.db
...
在你的 AI 客户端（Cursor / Claude Desktop / GitHub Copilot 等）的 MCP 配置中添加（路径替换成你自己的）。也可以先运行 `python nocturne.py config` 打印模板：
...
```json
{
  "mcpServers": {
    "nocturne_memory": {
      "command": "python",
      "args": ["C:/your/actual/path/to/nocturne_memory/nocturne.py", "stdio"]
    }
  }
}
```
...
```powershell
claude mcp add-json -s user nocturne-memory '{"type":"stdio","command":"python","args":["C:/absolute/path/to/nocturne_memory/nocturne.py","stdio"]}'
claude mcp list
```

> 看到 `nocturne-memory` 状态为 `Connected` 即成功。
> 如果你在安装向导里选择了可选 skill，Claude Code 还会自动拿到会话启动时的 `system://boot` 协议。
...
> [!TIP]
> 默认配置已经把数据库放到 `~/.nocturne-memory/memory.db`，不会再直接使用仓库内的 `demo.db`。

- **从零养成** — 保持默认 `memory.db`，或把 `.env` 里的 `DATABASE_URL` 改到你自己的 SQLite 文件，重启 MCP 即可。
- **基于已有数据库起步** — 复制你现有的 `.db` 文件到新位置，然后把 `DATABASE_URL` 指向它。
- **复用仓库里的 Demo** — 如果你想继承 `demo.db` 的预置人格，先复制一份到自己的数据目录，再把 `DATABASE_URL` 指向新文件；不要直接长期使用仓库里的 `demo.db`。
- **多设备共享** — 将 `DATABASE_URL` 改成 PostgreSQL 连接串，重启 MCP。
...
```json
{
  "mcpServers": {
    "nocturne_memory_alice": {
      "command": "python",
      "args": ["C:/path/to/nocturne_memory/nocturne.py", "stdio"],
      "env": { "NAMESPACE": "alice" }
    },
    "nocturne_memory_bob": {
      "command": "python",
